...
void Export2DB::addGeometry(
    ...
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
            std::cout << "   Something went wrong when adding the geomtery column in Table "
                << full_table_name(table) << ".\n"
                << sql  //   PQerrorMessage(mycon)
                << std::endl;
            PQclear(result);
        } else {
            std::cout << "   the_geom column added in table "
                << table
                <<  std::endl;
            PQclear(result);
        }
